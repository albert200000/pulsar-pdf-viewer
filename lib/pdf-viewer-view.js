...
export default class PdfViewerView {
  constructor(pathname) {
    ...
    const viewerPathname = path.join(
      atom.packages.getLoadedPackage('pulsar-pdf-viewer').path,
  ...
  synctexExit(code, synctex) {
    if (code == 0) {
      console.log('PdfViewerView: Opening SyncTeX result: line '
      ...
      console.log('PdfViewerView: SyncTeX failed with code ' + code)
    }
  }
  ...
  reloadPdf() {
    ...
    if (win) {
      ...
      const overrideFingerprint =
        atom.config.get('pulsar-pdf-viewer.overrideFingerprint')
      ...
      console.log('PdfViewerView: cannot (yet) reload PDF')
    }
  }
